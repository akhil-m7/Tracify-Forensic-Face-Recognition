...
public class face_rekognition extends javax.swing.JFrame {
    ...
    private void open_sketchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_open_sketchActionPerformed
        JFileChooser fileChooser = new JFileChooser("C:\\Users\\Temp\\OneDrive\\Desktop\\project\\Project Code (forensic face sketch)\\ThirdEye_FaceMatch\\src\\main\\java\\com\\mycompany\\thirdeye_facematch\\sketches"); 
    ...
    private void upload_sketchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_upload_sketchActionPerformed
        ...
        String bucketName = "tracifybuckets";
    ...
    for (FaceMatch face : faceImageMatches) {
        try {
            ...
            // Get image file name from AWS Rekognition response
            String matchedFileName = face.getFace().getExternalImageId().trim();
            match_path.setText(matchedFileName);

            // Construct the correct S3 image URL
            String path = "https://" + bucket + ".s3.us-east-1.amazonaws.com/" + matchedFileName;
            ...
            match_properties.setText(
                ...
                "FACE MATCHED\n" +
                "\n" +
                "\nName in database: " + matchedFileName + "\n" +
                "Similarity: " + face.getSimilarity() + "\n" +
                "Confidence: " + face.getFace().getConfidence() + "\n"
            );

            // Load image from URL and scale it
            try {
                URL url = new URL(path);
                ImageIcon imageIcon = new ImageIcon(url);
                Image imageFromUrl = imageIcon.getImage().getScaledInstance(match.getWidth(), match.getHeight(), Image.SCALE_SMOOTH);
                match.setIcon(new ImageIcon(imageFromUrl));
            } catch (IOException e) {
                System.err.println("Error loading image: " + e.getMessage());
                JOptionPane.showMessageDialog(null, "Error loading matched image.");
            }
